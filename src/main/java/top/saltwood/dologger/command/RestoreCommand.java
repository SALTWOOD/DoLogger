...
public final class RestoreCommand {
    ...
    private static int preview(CommandSourceStack source, String filters) {
        ...
        if (filters == null || filters.isBlank()) {
            source.sendFailure(LanguageResolver.component("dologger.commands.restore.empty_filters"));
            return 0;
        }
        ...
        source.sendSuccess(() -> LanguageResolver.component("dologger.commands.restore.preview", plannedIds.size()), false);
        source.sendSuccess(() -> LanguageResolver.component("dologger.commands.restore.confirm_hint"), false);
    ...
    private static int confirm(CommandSourceStack source) {
        ...
        if (plan == null || plan.isExpired()) {
            ...
            source.sendFailure(LanguageResolver.component("dologger.commands.restore.no_pending"));
            return 0;
        }
        PENDING_PLANS.remove(player.getUUID());
        ...
        if (level == null) {
            source.sendFailure(LanguageResolver.component("dologger.commands.restore.dimension_unavailable"));
            return 0;
        }

        RestoreCounts counts = executePlan(services, player, level, plan.entryIds());
        source.sendSuccess(() -> LanguageResolver.component("dologger.commands.restore.confirmed", counts.success(), counts.skipped(), counts.conflict()), true);
        return counts.success();
    }

    private static int cancel(CommandSourceStack source) {
        if (!Permissions.canRestore(source)) {
        ...
        PENDING_PLANS.remove(player.getUUID());
        source.sendSuccess(() -> LanguageResolver.component("dologger.commands.restore.cancelled"), false);
        return 1;
    }
    ...
    private static boolean validateRawActionFilter(CommandSourceStack source, String filters) {
        ...
        for (String token : filters.trim().split("\\s+")) {
            ...
            for (String action : token.substring(separator + 1).split(",")) {
                ...
                if (!SUPPORTED_ACTIONS.contains(normalized)) {
                    source.sendFailure(LanguageResolver.component("dologger.commands.restore.unsupported_action", action));
                    return false;
                }
        ...
        if (requested.isEmpty()) {
            source.sendFailure(LanguageResolver.component("dologger.commands.restore.action_required"));
            return false;
        }
        return true;
    }

    private static RestoreCounts executePlan(Services services, ServerPlayer player, Level level, List<Integer> entryIds) {
        int success = 0;
        int skipped = 0;
        int conflict = 0;
    ...
    private record RestoreCounts(int success, int skipped, int conflict) {
    }
}
